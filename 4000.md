...
Loads FAC2 with the five-byte floating-point value pointed to
by the accumulator and Y register (low byte/high byte), then
multiplies the value in FAC2 by the one in FACt, leaving the
results in FACt.
...
Loads FAC2 with the five-byte floating-point value pointed to
by the accumulator and Y register (low byte/high byte), then
divides the value in FAC2 by the one in FACt, leaving the results in FAC1.
...
### Determines the sign of the value in FACt
...
### Compares FACt against FAC2

## 36039 $8CC7 <a name="8CC7"></a>
### Converts FACt to a four-byte integer
