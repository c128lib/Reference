...
The CINT screen editor initialization routine [$C07B](C000#C07B) sets
...
routine [$C07B](C000#C07B) sets bits 1-3 to %100 in location 2605/$0A2D,
...
CINT screen editor initialization routine [$C07B](C000#C07B) sets bits 4-7
...
[$D01A](#D01A) has been set to trigger an external interrupt request for the event. Once a bit here is set to %1, it will retain
...
count equals the value in the raster compare register [$D012](#D012) (plus bit 7 of [$D011](#D011)).
...
The register at [$D01F](#D01F) records which sprites are involved in collisions with foreground data.
...
are represented by %0 or %00 bit patterns). The register at location [$D01E](#D01E) records which sprites are involved in collisions with other sprites.

**Bit 3**: This bit will be set to %1 whenever a new value is
latched into the light pen registers at [$D013-$D014](#D013).
...
internal VIC interrupts, recorded in the register at [$D019](#D010). The VIC chip has the capability to generate an external interrupt request to the processor as a result of any of
...
bit 0 of the register at [$D019](#D019). Because raster interrupts
...
at bit 1 of the register at [$D019](#D019), as when a sprite
...
bit 2 of the register at [$D019](#D019).
...
register at [$D019](#D019). It is risky to use this interrupt
...
bits is the same as for the sprite enable register ([$D015](#D015)). While a bit here is %0, the corresponding sprite will
...
is the same as for the sprite enable register ([$D015](#D015)).
...
%01 and %11 bit patterns take their color values from the registers at [$D025](#D025) and [$D026](#D026), respectively, which
are common to all multicolor sprites. Pixels with %10 bit patterns take the color specified in the color register for the particular sprite ([$D027-$D02E](#D027)).
...
([$D015](#D015)). Setting a bit here to %1 will double the horizontal width of the corresponding sprite. Each sprite can be
...
register at [$D017](#D017).) The default value in this register,
...
the interrupt register at [$D019](#D019) will also be set to %1 whenever any sprite-sprite collision occurs.
...
the interrupt register at [$D019](#D019) will also be set to %1
...
the register at [$D011](#D011) to %0, the entire display area will
...
[$D021](#D021), and the color of pixels with %11 bit pairs is selected individually for each character position by the value in
...
[$E019](E000#E019), part of the reset and RUN/STOP-RESTORE sequences. The default values are 1 (white) for 53282/$D022, 2
...
mode by setting the appropriate bit in the register at [$D01C](#D01C). The value in the register at 53285/$D025 determines
...
[$D027-$D02E](#D027). Pixels with %00 bit pairs in the
...
It is common practice to set bit 4 of the VIC register at [$D011](#D011) to %0 to blank the 40-column screen display while operating in 2-MHz mode. For example, the BASIC routine for
...
bit. The IOINIT routine [$E019](E000#E019) sets the bit to %0, and no
other 128 ROM routine changes that setting. Some programmers have discovered that setting this bit to %1 will blank the
40-column screen display, and have even used this as an alternative to clearing bit 4 of location [$D011](#D011) when switching the processor to fast mode. While this does appear to work
