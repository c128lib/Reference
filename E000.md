...
[$EEBO] is called; then raster interrupts are reenabled, and the
...
character in the accumulator [$C00C]. If a serial device is specified (device number greater than 3), the routine jumps to the
...
routine [$FFCC] is called to reset the default IjO channels. Bit
...
is set before CLOSE [$FFCC] is called so that the special command file close is performed.) The status register carry bit will
...
Reads the CIA port connecting the rows of the keyboard matrix. The keyboard scan routine [$C55D], normally performed
...
the Kernal CLRCH routine [$FFCC] is called to restore default
...
and DMA_CALL routines. The routines are copied to the appropriate areas of RAM by the routine at 57549/$E0CD, part
...
the location immediately following the J5R which called this
...
address following the zero byte rather than to the address following the calling J5R.

It's very important always to call this routine (or its jump
table entry) with J5R, not JMP. Only J5R puts a return address
...
* A call to the 10lNIT routine [$E109] to reset all I/O chip registers to their default values (character definitions for 80-
column video are normally not reinitialized).
* A call to the CINT routine [$C000] to restore screen editor
...
routine [$C024] is called to handle screen mode settings, scan
...
[$F5F8] is called to update the jiffy timers; then the tape motor
interlock handling routine [$EED0] is called. The interrupt
...
## 65280-65284 $FF00-$FF04 MMU Registers <a name="FF00"></a>
The MMU configuration and load configuration registers appear here in all banks.
