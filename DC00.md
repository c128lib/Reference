...
[$DC00-$DC01](#DC00). For lines PB6-PB7 of port B, the settings of
...
of the control registers at [$DC0E-$DC0F](#DC0E)
...
0 of the CIA interrupt register at [$DC0D](#DC0D) will be set to
...
the 16-bit counter (low byte in $DC04 and high byte in
...
([$DC0C](#DC0C)) for more information on the CIA's serial data
I/O capabilities.
...
controlled by the register at [$DC0F](#DC0F)
...
control register B ([$DC0F](#DC0F)) determines whether the
...
CIA interrupt register ([$DC0D](#DC0D)). At this point,
...
register ([$DC0D](#DC0D)) to indicate that another byte can
be sent.
...
[$FA65](E000#FA65) merely reads this register to clear it, and ignores the
...
6-7, which control serial data port and time-of-day clock functions,
 respectively. See the entry at [$DC04-$DC05](#DC04)
