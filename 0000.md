...
has a leading negative sign. During the series evaluation routine [$909C], this location holds the number of terms in the
series.
...
$6B-$6E are the mantissa. All operations that involve both accumulators will transfer the results to FAC1.
...
[104/$68](#68) and 111/$6F—the signs of the values in the respective
...
[$A778] will set the values here whenever the DS or DS$
variables are used.
...
used to hold information for FOR, GOSUB, and DO statements (see Chapter 3 for details). The value here is the address of the next free location in the stack, which is filled from
...
retrieved from the stack, the value is incremented by the number of bytes to be removed.
...
2. The value here doesn't have any immediate effect on the
...
If the parameter is omitted, a default value of 12800/13 200 is
...
||64/$40|EOF (end of file)|end-of-file marker read|
...
SCNKEY routine [$C55D], normally performed earlier in the
...
(indicated by a nonzero value), or whether bits are still being read
...
from the instruction address buffer (2720-2729/
$0AA0-$0AA9).
...
times per second.) Thus, location 162/$A2 will be incremented
...
When characters are being received over the RS-232 interface, this location is used as a countdown for the number of
...
the operation ending address at 174-175/
$AE-$AR There is also a routine [$F7CC] to retrieve the character at
...
routine [$E1FO] to initialize the soft reset vector,
...
checks on the shift-key status (in location 211/$D3) and selects
the proper value from the list of keyboard table pointers
...
This location is set during the SCNKEY routine [$C55D] to indicate
...
functions. Location 218/$DA is used as temporary storage by
...
219/$DB holds the total length of all programmable key definitions.
Location 220/$DC holds the current key number
(0-9). Location 221/$DD holds the index to the next key definition
beyond the current one. Location 222/$DE holds the
...
231/$E7. The value here is reset to 0/$00, the left edge of the
...
$E0-$E1) and the starting address for the current color memory
line (226-227/$E2-$E3). When the output window is
...
the value in location 230/$E6, the left margin of the output
...
$E6. The value here is also reset to the left margin value
whenever a RETURN character (code 13/$0D) or SHIFT-RETURN
...
the value here less the current left margin value in 230/$E6.
...
to the value here. During the CINT screen editor initialization
...
be printed. Bit 4 determines whether
...
attribute. Bit 6 could be used to determine whether
...
value here is exclusive-ORed with l/$01 each time a quote
...
this bit is %0, the SCNKEY routine [$C55D] will switch character
...
sets by printing character 14/$0E for the lowercase/
...
code 11/$OB, and reset to %0 by printing character code
...
is %0, linking is allowed. The line link bitmap at 862-865/
